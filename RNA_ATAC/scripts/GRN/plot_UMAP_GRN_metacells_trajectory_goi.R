...
p$add_argument('--goi_name',       type="character",               help='Gene of interest name')
...
goi_list <- list(shortlist=c("SPIC", "TBX3", "ESRRB", "ETV1", "POU2F3", "SOX11", "ONECUT1", "MEIS1", "HIVEP3", "ID2", "KLF4", "SP8", "IRX4", "HOXB2", "PBX3", "HNF4G"),
                 longerlist=c("SPIC", "TBX3", "ESRRB", "ETV1", "POU2F3", "SOX11", "ONECUT1", "MEIS1", "HIVEP3", "ID2", "KLF4", "SP8", "IRX4", "HOXB2", "PBX3", "HNF4G",
                              "TCF4", "RORB","GLI2", "ONECUT2", "RUNX1", "NR5A2"),
                 evenlongerlist=c("SPIC", "TBX3", "ESRRB", "ETV1", "POU2F3", "SOX11", "ONECUT1", "MEIS1", "HIVEP3", "ID2", "KLF4", "SP8", "IRX4", "HOXB2", "PBX3", "HNF4G", 
                                  "TCF4", "RORB", "GLI2", "ONECUT2", "RUNX1", "NR5A2" , "TCF15", "SOX3", "GATA4", "LHX8"),
                 longerthanevenlonglist=c("SPIC", "TBX3", "ESRRB", "ETV1", "POU2F3", "SOX11", "ONECUT1", "MEIS1", "HIVEP3", "ID2", "KLF4", "SP8", "IRX4", "HOXB2", "PBX3", "HNF4G",
                                          "TCF4", "RORB", "GLI2", "ONECUT2", "RUNX1", "NR5A2" , "TCF15", "SOX3", "GATA4", "LHX8", "KLF5", "SOX12", "ELF3", "CREB5", "ARID5B"))

print("Goi lists")
print(names(goi_list))
print(args$goi_name)

if(args$goi_name%in%names(goi_list)){
  args$goi <- goi_list[[args$goi_name]]
} else {
  stop("Please add your genes of interest in the 'plot_UMAP_GRN_metacells_trajectory_goi.R' script!")
}
...
# set to goi
markers_TF_undupl <- markers_TF_undupl[markers_TF_undupl$TF%in%args$goi,]
if (sum(!(args$goi%in%markers_TF_undupl$TF))>0){
  not_in_marker_TFs <- cbind(args$goi[!(args$goi%in%markers_TF_undupl$TF)],rep(6,sum(!(args$goi%in%markers_TF_undupl$TF))))
  colnames(not_in_marker_TFs) <- colnames(markers_TF_undupl)
  markers_TF_undupl <- rbind(markers_TF_undupl,not_in_marker_TFs)
}
...
if (!file.exists(file.path(args$outdir,sprintf("Score%s/betas_mtx.txt",args$min_chip_score))) | args$force){
  ...
  for (i in 1:ncol(betas_mtx)){
    if((i%%1000)==0){print(i)}
    if (sum((GRN_coef.dt$tf==colnames(betas_mtx)[i]) & (tf2gene_chip.dt$tf==colnames(betas_mtx)[i]))>0){
...
# ##########################################################
# ## Plot outgoing activatory per cluster on general plot ##
# ##########################################################
...
#       } else {
#         celltypes_markers[!((V(igraph.tbl_marker)$tf%in%GRN_coef.dt$tf[(GRN_coef.dt$tf%in%TFs_marker_j) & (GRN_coef.dt$gene%in%TFs_marker_j) & (GRN_coef.dt$beta<0)]) |
#                               (V(igraph.tbl_marker)$tf%in%GRN_coef.dt$tf[(GRN_coef.dt$tf%in%TFs_marker_j) & (GRN_coef.dt$gene%in%TFs_marker_j) & (GRN_coef.dt$beta<0)]))] <- NA
#       }
...
#       } else {
#         celltypes_markers[!((V(igraph.tbl_marker)$tf%in%GRN_coef.dt$tf[(GRN_coef.dt$tf%in%TFs_marker_j) & (GRN_coef.dt$gene%in%TFs_marker_jand1) & (GRN_coef.dt$beta<0)]) |
#                               (V(igraph.tbl_marker)$tf%in%GRN_coef.dt$tf[(GRN_coef.dt$tf%in%TFs_marker_j) & (GRN_coef.dt$gene%in%TFs_marker_jand1) & (GRN_coef.dt$beta<0)]))] <- NA
#       }
...
# cluster_names <- c("Naïve","Early partially primed","Late partially primed","Early primed","Late primed")
# for (j in 1:5){
#   print(paste0("Plot global network for cluster ",j))
...
# setwd("C:/Users/Louis Coussement/Downloads/Score0.06")
# 
# prefixes <- c("global_network_gradient_expr_perc",
#               "global_network_gradient_zscore",
#               "global_network_gradient_zscore_red")
# 
# ## White background
...
# 
#   image_write(animation,paste0("C:/Users/Louis Coussement/Downloads/",j,"_cluster.gif"))
# 
# }
# 
# 
...
# for (j in prefixes){
#   files <- list.files()[grepl(paste0("^",j,"_sampled[0-9]+.pdf"),list.files())]
#   files <- files[sort(as.numeric(gsub(".pdf","",gsub(".*sampled","",files))),index.return=T)$ix]
#   print(files)
#   counter <- 1
#   for (i in 1:length(files)){
#     image_i <- image_read_pdf(files[i],density=600)
...
#   animation <- image_animate(images, fps = 1)
# 
#   image_write(animation,paste0("C:/Users/Louis Coussement/Downloads/",j,"_sample.gif"))
# }
# 
...
#   animation <- image_animate(images, fps = 1)
# 
#   image_write(animation,paste0("C:/Users/Louis Coussement/Downloads/",j,"_cluster_nb.gif"))
#   gc()
# }
# 
# 
...
#   animation <- image_animate(images, fps = 1)
# 
#   image_write(animation,paste0("C:/Users/Louis Coussement/Downloads/",j,"_sample_nb.gif"))
#   gc()
# }


# #########################################
# ## Plot outgoing links per gene in GRN ##
# #########################################

for (gene in args$goi){
   print(paste0("Plot global network for cluster ",gene))
  ...
  # print(length(E(igraph.net_marker)$weight))
  mask_pos <- (E(igraph.tbl_marker)$weight>0) & !grepl("^Not",E(igraph.tbl_marker)$sign)
  mask_not <- (!((edge_list.dt_marker$from%in%gene)))
  mask_neg <- (E(igraph.tbl_marker)$weight<0) & !grepl("^Not",E(igraph.tbl_marker)$sign)
  # print(sum(mask_pos & mask_not))
  # print(sum(mask_neg & mask_not))
  E(igraph.tbl_marker)$sign[mask_not] <- paste0("Not in cluster (n=",sum(mask_not),")")
  ...
  col_arrows <- NULL
  if (sum(mask_pos)-sum(mask_pos & mask_not)>0){
        col_arrows <- c(col_arrows,"darkred")
  } else {
     col_arrows <- c(col_arrows,"grey")
  }
  if (sum(mask_neg)-sum(mask_neg & mask_not)>0){
        col_arrows <- c(col_arrows,"darkblue")
  } else {
     col_arrows <- c(col_arrows,"grey")
  }
  col_arrows <- c(col_arrows,"grey")
  print(col_arrows)
  print(unique(as.character(E(igraph.tbl_marker)$sign)))
  ...
  # celltypes_markers[!(V(igraph.tbl_marker)$tf%in%gene)] <- NA
  ...
  p <- ggraph(igraph.tbl_marker, x=umap_embedding[names(V(igraph.net_marker)),2],
    ...
    pdf(file.path(args$outdir,sprintf("Score%s/%s_global_network_markerTFs.pdf",args$min_chip_score,gene)), 
        width = 6, height = 5.5)
    print(p)
    dev.off()
}
