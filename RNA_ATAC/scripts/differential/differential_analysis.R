...
# # # args$matrices <- c("RNA","PeakMatrix","GeneScoreMatrix_TSS","GeneScoreMatrix_distal")
# args$cluster_ids <- 1:5
# args$rm_prev_res <- TRUE
# # args$get_DE_genes <- TRUE
...
args$sce <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/pseudobulk_with_replicates.rds",
                    args$matrix)

args$groups <- combn(args$cluster_ids,2)

args$QC_dir <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/QC",
                           args$matrix)

dir.create(args$QC_dir, showWarnings=F)

if(args$rm_prev_res){
  args$outdir <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/DE_res",
                    args$matrix)
...
to.plot <- sce_logcount_filt %>% melt
colnames(to.plot) <- c("gene","pseudorep","logcount")
...
for (i in 1:ncol(args$groups)){
  ...
  args$outfile <-  sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/DE_res/res_cluster%svsclusters%s.txt",
  ...
  # BCV plot
  jpeg(sprintf("%s/BCVplot_cluster%svsclusters%s.jpg",args$QC_dir,args$groupA,args$groupB))
  plotBCV(sce_edger) 
  dev.off()

  ## MA plot
  jpeg(sprintf("%s/MAplot_cluster%svsclusters%s.jpg",args$QC_dir,args$groupA,args$groupB))
  ...
  ## Pvalue distribution
  jpeg(sprintf("%s/Pval_dist_cluster%svsclusters%s.jpg",args$QC_dir,args$groupA,args$groupB))
...
for (i in 5:10){
  ...
  pdf(file = sprintf("%s/heatmap_%sgeneclusters.pdf",
                     args$outdir, i), height = 6, width = 8)
  range_breaks <- seq(from=-2,to=2,length.out=101)
  exp.values.scaled %>%
    ...
    pheatmap(cluster_rows = F,
             ...
             color = colorRampPalette(cols3)(100),breaks=range_breaks)
  dev.off()
}
...
if (args$matrix=="RNA"){
  # order_gene_clusters <- c(4,2,1,6,3,7,5)
  order_gene_clusters <- c(3,1,7,5,2,6,4)
  n_clusters <- 7
} else if(grepl("Peak",args$matrix)){
  order_gene_clusters <- c(6,2,5,4,3,1)
  n_clusters <- 6
} else if(grepl("TSS",args$matrix)){ 
  order_gene_clusters <- c(3,5,1,6,4,2)
  n_clusters <- 6
} else if(grepl("distal",args$matrix)){ 
  order_gene_clusters <- c(6,2,4,1,5,3)
  n_clusters <- 6
}
pairw_comp <- paste0(combn(1:5,2)[1,],rep("vs",ncol(combn(1:5,2))),combn(1:5,2)[2,])
...
# # cluster specific when OE to at least 1 other cluster, specificy attributed to maximal expression value
# # !!!!! CHECK IF LOGFC GO IN THE RIGHT DIRECTION!
# for (i in 1:5){
#   cols_logFC <- which(grepl(i,c_names) & grepl("vs",c_names) & grepl("logFC",c_names))
#   cols_FDR <-  which(grepl(i,c_names) & grepl("vs",c_names) & grepl("FDR",c_names))
#   sign <- c(1,-1)[c(substr(gsub("[A-Z,a-z]+_","",c_names[cols_logFC]),1,1)==i)+1]
# 
#   DEG_table_sorted <- cbind(DEG_table_sorted,
#                             (rowSums((t(t(DEG_table_sorted[,cols_logFC])*sign)) &
#                                        (DEG_table_sorted[,cols_FDR]<0.01))>1) &
#                               (apply(DEG_table_sorted[,grepl("log_av",c_names)],1,which.max)==i))
#     c_names <- c(c_names,paste0("clust",i,"_spec"))
# }
...
# png
png(file = paste0(args$outdir,"/heatmap_",
                  n_clusters,"geneclusters_final.png"),
...
# png - nolabs
png(file = paste0(args$outdir,"/heatmap_",
                  n_clusters,"geneclusters_final_nolabs.png"),
    height = 6, width = 8, units="in", res=300)
exp.values.scaled %>%
  rownames_to_column(var="Gene") %>% 
  full_join(clusters.n_ordered) %>%
...
if (grepl("RNA",args$matrix)){
  ...
  for (t in c("GeneScoreMatrix_TSS","GeneScoreMatrix_distal")){
    print(t)
    args$sce_atac <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/pseudobulk_with_replicates.rds",
                             t)
    ...
    for (i in 1:5){
      ...
      # print(length(exp.values[,i]))
      # print(length(exp.values_atac[,i]))
      # print(dim(obj_i))
      # print(head(obj_i))
    ...
    colnames(cor_RNA_seq) <- paste0(rep("cluster",5),1:5)
    ...
    exp.values.scaled_multi.melted <- exp.values.scaled_multi %>% select(-Gene) %>% melt
    exp.values.scaled_multi.melted <- cbind(exp.values.scaled_multi.melted,
                                            rep(c("A","B","C","D","E","F","G")[clusters.n_ordered$cluster[sort(clusters.n_ordered$Gene,index.return=T)$ix]],5))
  ...
  args$sce_RNA <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/%s/pseudobulk_with_replicates.rds",
                          "RNA")
  ...
  colnames(exp.values_expr) <- paste0(rep("expr_cluster",5),1:5)
