...
to.plot.density_rna <- cbind(logcounts(rna_metacells.sce)%>% reshape2::melt(),rep("RNA",ncol(rna_metacells.sce)*nrow(rna_metacells.sce)))         
to.plot.density_atac <- cbind(assay(atac_peakMatrix_metacells.se,"logcounts")[peak2gene.dt$peak,]%>% reshape2::melt(),
