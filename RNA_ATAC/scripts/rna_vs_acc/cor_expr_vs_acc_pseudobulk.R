...
to.plot.density_rna <- cbind(logcounts(rna_pseudobulk.sce)%>% reshape2::melt(),rep("RNA",ncol(rna_pseudobulk.sce)*nrow(rna_pseudobulk.sce)))         
to.plot.density_atac <- cbind(assay(atac_peakMatrix_pseudobulk.se,"logcounts")[peak2gene.dt$peak,]%>% reshape2::melt(),
