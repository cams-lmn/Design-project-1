...
# args$atac_peak_matrix <- "/data/louisc/Project_Babraham/RNA_ATAC/pseudobulk/cluster/PeakMatrix/pseudobulk_with_replicates.rds"
# args$peak_metadata <-"/data/louisc/Project_Babraham/ATAC/archR/peak_metadata.tsv.gz"
# args$tf2peak_cor <- sprintf("/data/louisc/Project_Babraham/RNA_ATAC/rna_vs_acc/pseudobulk/TFexpr_vs_peakAcc/%s_cor_TFexpr_vs_peakAcc.rds",args$motif_annotation)
# args$motifmatcher <- sprintf("/data/louisc/Project_Babraham/ATAC/archR/Annotations/%s-Scores.rds",args$motif_annotation)
# args$motif2gene <- sprintf("/data/louisc/Project_Babraham/ATAC/archR/Annotations/%s_motif2gene.txt.gz",args$motif_annotation)
# # args$max_acc_score <- 4.5
# # args$min_peak_score <- 15
# args$min_number_peaks <- 50
# args$outdir <- sprintf("/data/louisc/Project_Babraham/RNA_ATAC/virtual_chipseq/pseudobulk/%s/",args$motif_annotation)
# args$threads <- 10
# ## END TEST ##
...
virtual_chip.dt <- opts$TFs %>% map(function(i) {
  ...
  if (length(peaks)>=args$min_number_peaks) {
    ...
    # max_accessibility_score <- apply(assay(atac_peakMatrix_pseudobulk.se[peaks,]),1,max) %>% round(2)
...
# virtual_chip.mtx <- readRDS(file.path(args$outdir,"virtual_chip.mtx"))
...
# motifmatcher_chip.se <- motifmatcher.se
# assays(motifmatcher_chip.se) <- assays(motifmatcher_chip.se)[c("motifMatches")]
# assay(motifmatcher_chip.se,"motifMatches")[assay(motifmatcher_chip.se,"motifMatches")==1] <- 0
# stopifnot(rownames(virtual_chip.mtx)%in%rownames(motifmatcher_chip.se))
# 
# # i <- "CLOCK"
# for (i in colnames(virtual_chip.mtx)) {
#   print(sprintf("%s (%s/%s)",i,match(i,colnames(virtual_chip.mtx)),ncol(virtual_chip.mtx)))
#   peaks <- which(abs(virtual_chip.mtx[,i])>=0) %>% names
#   assay(motifmatcher_chip.se,"motifMatches")[,i][rownames(motifmatcher_chip.se)%in%peaks] <- TRUE
# }
# 

# assay(motifmatcher.se["chr5:4894870-4895470","T"],"motifScores")
# sum(assay(motifmatcher.se[,"T"],"motifMatches")[,1])
