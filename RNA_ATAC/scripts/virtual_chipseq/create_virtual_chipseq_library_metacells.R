...
# args$atac_peak_matrix <- "/data/homes/louisc/Project_Babraham/RNA_ATAC/metacells/cluster/PeakMatrix/metacells_with_replicates.rds"
# args$peak_metadata <-"/data/homes/louisc/Project_Babraham/ATAC/archR/peak_metadata.tsv.gz"
# args$tf2peak_cor <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/rna_vs_acc/metacells/TFexpr_vs_peakAcc/%s_cor_TFexpr_vs_peakAcc.rds",args$motif_annotation)
...
# args$outdir <- sprintf("/data/homes/louisc/Project_Babraham/RNA_ATAC/virtual_chipseq/metacells/%s/",args$motif_annotation)
# args$threads <- 10
# ## END TEST ##
...
stopifnot(rownames(tf2peak_cor.se)%in%rownames(atac_peakMatrix_metacells.se))
...
atac_peakMatrix_metacells.se <- atac_peakMatrix_metacells.se[peaks,]
...
atac.mtx <- assay(atac_peakMatrix_metacells.se[peaks,],"logcounts") %>% round(3)
...
virtual_chip.dt <- opts$TFs %>% map(function(i) {
  ...
  if (length(peaks)>=args$min_number_peaks) {
    ...
    # max_accessibility_score <- apply(assay(atac_peakMatrix_metacells.se[peaks,]),1,max) %>% round(2)
