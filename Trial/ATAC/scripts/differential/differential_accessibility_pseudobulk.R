...
# args$seed1 <- 42
# args$threads <- 1
# args$cluster_ids <- c("C1", "C2", "C3", "C4", "C5")
# args$peak_metadata <- "/data/homes/louisc/Project_Babraham/ATAC/archR/PeakCalls/peak_metadata.tsv.gz"
# ## END TEST
...
args$atac_matrix_file <- sprintf("/data/homes/louisc/Project_Babraham/ATAC/archR/pseudobulk/%s/%s/pseudobulk_with_replicates.rds",
                                 args$group_by,args$matrix)

args$groups <- combn(args$cluster_ids,2)

args$outdir <- sprintf("/data/homes/louisc/Project_Babraham/ATAC/archR/pseudobulk/%s/%s/DE_res", args$group_by,args$matrix)
...
if (args$matrix=="PeakMatrix") {
  #order_gene_clusters <- c(5,2,3,4,1,6)
  order_gene_clusters <- c(6,3,4,5,2,1)
} else if (grepl("GeneScoreMatrix_TSS",args$matrix)) {
  order_gene_clusters <- c(5,6,2,4,3,1)
} else  if (grepl("GeneScoreMatrix_distal",args$matrix)){
  order_gene_clusters <- c(2,5,6,1,4,3)
}
...
# png
png(file = sprintf("%s/heatmap_%sgeneclusters_final.png",args$outdir,n_clusters),
...
# png - nolabs
png(file = sprintf("%s/heatmap_%sgeneclusters_final_nolabs.png",args$outdir,n_clusters),
    height = 6, width = 8, units="in", res=300)
exp.values.scaled %>%
  rownames_to_column(var="Gene") %>%
  full_join(clusters.n_ordered) %>%
